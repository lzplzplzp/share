...
import com.code.util.image.ImageUtils.Position;
...
//		}
	...
	/**
	 * 从BufferedImage里生成缩略图
	 * @param srcImgBuff  BufferedImage的原始图片
	 * @param destFile   目标图片
	 * @param boxWidth   缩略图最大宽度
	 * @param boxHeight  缩略图最大高度
	 * @throws IOException
	 */
	public static void resizeFix(BufferedImage srcImgBuff, File destFile, int boxWidth,
			int boxHeight) throws IOException {
		int width = srcImgBuff.getWidth();
		int height = srcImgBuff.getHeight();
		int zoomWidth;
	...
	public static void writeFile(BufferedImage imgBuf, File destFile)
		...
		String tempName[] = destFile.getName().split("\\.") ;
		if(tempName!=null&&tempName.length==2){
			try{
				ImageIO.write(imgBuf,tempName[1], destFile);
			}catch(Exception e){
				ImageIO.write(imgBuf, "jpg", destFile);
			}	
		}else{
			ImageIO.write(imgBuf, "jpg", destFile);
		}
...
//				"D:/tmp/favicon.png"), new File(
//				"D:/tmp/favicon_100.png"), 100, 100);
//		AverageImageScale.resizeFix(new File(
//				"D:/tmp/favicon.png"), new File(
//				"D:/tmp/favicon_75.png"), 75, 75);
		AverageImageScale.resizeFix(new File(
				"D:/tmp/3.jpg"), new File(
				"D:/tmp/3_3.jpg"), 760, 580);
		System.out.println("ok");
//		File old = new File(
//				"D:/temp/3.jpg") ;
//		AverageImageScale.resizeFix(old, new File(
//				"D:/temp/3_1.jpg"), 1632, 1224);
//		time = System.currentTimeMillis() - time;
//		System.out.println("resize2 img in " + time + "ms");
		
//		 imageMark(BufferedImage imgBuff, int width, int height,
//					int pos, int offsetX, int offsetY, String text, Color color,
//					int size, int alpha)
	}
}